...
for contents in input:
    same_item = "*"
    rucksack_size = len(contents) // 2

    for item1 in contents[0:rucksack_size]:
        for item2 in contents[rucksack_size:-1]:
            if item1 == item2:
                same_item = item1
                priority_sum += priority(item1)
                break
        if (same_item != '*'):
            break

    print(f"The rucksack has {contents[0:-1]} in it.")
    print(f"The first compartment has {contents[0:rucksack_size]},")
    print(f"and the second compartment has {contents[rucksack_size:-1]}.")
    print(
        f"Both compartments have {same_item} in them, with a priority of {priority(same_item)}\n")

input.close()

print(f"The sum of the duplicate item priorities is {priority_sum}.\n")
