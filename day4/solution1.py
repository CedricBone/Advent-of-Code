...
for pair in input:
    ...
    elf1_contain_elf2 = (elf1_start <= elf2_start) and (elf1_end >= elf2_end)
    elf2_contain_elf1 = (elf2_start <= elf1_start) and (elf2_end >= elf1_end)

    if (elf1_contain_elf2 or elf2_contain_elf1):
        fully_contain_count += 1

input.close()
print(
    f"\nThere were {fully_contain_count} assignment pairs where one range fully contain the other.\n")
